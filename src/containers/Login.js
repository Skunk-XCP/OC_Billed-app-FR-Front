...
export default class Login {
   ...
   handleSubmitAdmin = (e) => {
      ...
      const user = {
         type: "Admin",
         email: e.target.querySelector(`input[data-testid="admin-email-input"]`) //La ligne a été copiée à partir de la ligne "employee" sans avoir été modifiée
            .value,
         password: e.target.querySelector(
            `input[data-testid="admin-password-input"]` // Meme constat que pour la data EMAIL
         ).value,
         status: "connected",
      };
      this.localStorage.setItem("user", JSON.stringify(user));
